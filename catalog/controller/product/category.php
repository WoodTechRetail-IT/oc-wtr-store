	...
	public function index() {
		...
		if ($category_info) {
			...
			if ($category_info['imagetwo']) {
				$data['thumbtwo'] = $this->model_tool_imagequadr->resize($category_info['imagetwo'], 120, 80);
			} else {
				$data['thumbtwo'] = '';
			}

			$data['description'] = html_entity_decode($category_info['description'], ENT_QUOTES, 'UTF-8');
			$data['category_dopcontblock'] = html_entity_decode($category_info['category_dopcontblock'], ENT_QUOTES, 'UTF-8');
			$data['category_dopsku'] = html_entity_decode($category_info['category_dopsku'], ENT_QUOTES, 'UTF-8');
			$data['category_doptype'] =$category_info['category_doptype'];
			$data['category_dopcontblocksecond'] =$category_info['category_dopcontblocksecond'];
			...
			foreach ($results as $result) {
    				if ($result['pmtemplate'] == 'productpol') {
                        ...
                        if ($result['image']) {
                            $image = $this->model_tool_image->resize($result['image'], 112, 228);
                        } else {
                            $image = $this->model_tool_image->resize('placeholder.png', $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_width'), $this->config->get('theme_' . $this->config->get('config_theme') . '_image_product_height'));
                        }
